...
For real world publication quality graphics, it is likely that the outputs will need to be visually tweak or adjusted.

## Basics: How to Use this Package

The package should first be installed using the usual paths (`install.packages`) from GitHub or elsewhere; the package can then be loaded using (`library(spatialcompare)`)
...
```{r }
library(sf)
nc_original <- st_read(system.file("shape/nc.shp", package="sf"))
...
We are going to compare the different dimensions of the North Carolina counties side-by-side. The first step is to transform the `sf` features (in this case the county shapes) into a "normalized" location so that the centre of each feature is at Easting 0m and Northing 0m. 

For this we use the `normalizeAroundCentroid` function. In some instances we might want to transform multiple features around a single centroid for the collection of features.

```{r }
nc_normal_combined <- normalizeAroundCentroid(nc,by.feature=F)
plot(nc_normal_combined,axes=T)
abline(v=0,col="red")
abline(h=0,col="green")
```

However, since we are specifically interested in comparing the county shapes and sizes, we would like to transform each county individually, so set `by.feature` to `TRUE`. Here you can see that all of the counties are now overlapping around `0,0`.

```{r }
nc_normal <- normalizeAroundCentroid(nc,by.feature=T)
plot(nc_normal,axes=T)
abline(v=0,col="red")
abline(h=0,col="green")
```
...
Currently only one form of distribution is available, a regular grid based on the maximal bounding box of all features to calculate grid size. Future versions may allow better fitting of irregular features. This is the default type of distribution algorithm.

```{r }
par(mar=c(0,0,0,0))
plot(distribute(nc_normal),col=nc_normal$AREA)
```
...
```{r }
par(mar=c(0,0,0,0))
plot(distribute(nc_normal, margin=1.5))
plot(distribute(nc_normal, margin=1, x.mar=2))
```
...
```{r }
par(mar=c(0,0,0,0))
plot(distribute(nc_normal, scale=0.3))
```
...
```{r }
par(mar=c(0,0,0,0))
plot(distribute(nc_normal, dir="h", rows=10))
```
...
```{r }
par(mar=c(0,0,0,0))
plot(distribute(nc_normal[rev(order(nc_normal$AREA)),], max.features=20, margin=2),
...
```{r }
par(mar=c(0,0,0,0))
nc_n2 <- normalizeAroundCentroid(nc_original,by.feature=T)
...
```{r }
par(mar=c(0,0,0,0))
plot(st_geometry(distribute(nc_normal_combined)), col = sf.colors(20, categorical = TRUE), border = 'grey')
```
