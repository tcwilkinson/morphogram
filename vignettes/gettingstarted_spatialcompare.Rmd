---
title: "Getting Started with `spatialcompare`: Ordered Tableaux for Simple Features"
...
However, since we are specifically interested in comparing the county shapes and sizes, we would like to transform each county individually, `by.feature` should be `TRUE` (the default). Here you can see that all of the counties are now overlapping around `0,0`.

```{r }
nc_converged <- converge(nc) # which is the same as:   converge(nc,by.feature=T)
plot(nc_converged,axes=T)
```
...
## Visualizing the relative size of countries 
...
```{r }
library("rnaturalearth")
urban_areas_src <- ne_download(scale = 50, type = "urban_areas", load = TRUE, returnclass="sf")
urban_areas <- urban_areas_src[3]

populated_places_src <- ne_download(scale = 50, type = "populated_places", load = TRUE, returnclass="sf")
...
## Visualising the growth of cities
